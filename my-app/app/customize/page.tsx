...
export default function CustomizePage() {
  ...
  const handleOpenInNewTab = () => {
    ...
    if (selectedTemplate && selectedTemplate.html) {
      // Inject background image script into the HTML
  ...
  const handleGenerate = async (prompt: string) => {
    ...
    try {
      ...
      const fileName =
        selectedTemplate?.title?.replace(/\s+/g, "_").toLowerCase() ||
        "template_image";
    ...
    try {
      ...
      const res = await fetch(
        process.env.NEXT_PUBLIC_BACKEND_URL + "/createimage",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ imageUrl: url, fileName, id }),
        }
      );
      const data = await res.json();
      setSelectedTemplate((prev) => {
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedTemplate.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-1 text-xs bg-gray-800 text-gray-300 rounded-md font-mono"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-400 text-sm font-mono">
                Describe the changes you want to make to this template. Our AI
                will generate the updated version for you.
              </p>
            </div>

            <PromptEditor
              onVisualGenerate={handleVisualGenerate}
